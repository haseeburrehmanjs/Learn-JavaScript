...
let userNames = "Haseeb Ur Rehman"
alert(userNames)
...
let numbers = +prompt('Enter Your Table Number');
let lengths = +prompt('How many Times Do you want?');

for(let number = lengths; number <= lengths; number++){
    for(let j = 1; j <= lengths; j++){
    console.log(`${numbers} x ${j} = ${numbers*j}`);
    }
}
...
let userNumbers = prompt('Enter Your Number!')
